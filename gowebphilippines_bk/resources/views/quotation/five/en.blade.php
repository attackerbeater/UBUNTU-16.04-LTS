  ...
  <form method="post" action="{{url('vendor/unisharp/laravel-ckeditor/process.php')}}">
    ...
    <textarea cols="80" class="ckeditor" name="editor" id="editor-en" rows="10">
      ...
      @if($supplier[0]->supplier_exclusivity_status == 1)        
        ...
        <p><u><strong>ORDER CONFIRMATION : {{ strtoupper($order[0]->order_subject) }}</strong></u></p>
        ...
        <p>Number of packages: {{ $output['no_of_packages'] }}<br/>
        Package size: <br/>
        Weight: {{ $output['weight'] }} lbs. <br/>
        Value of goods EUR: <br/>
        Country of origin:  {{$supplier[0]->supplier_country}} <br/>
        Goods/Tariff number:  <br/>
        Delivery terms:     </p>
